...
export const CreatePost = ({ communities }: {
    ...
    return <>
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 overflow-y-scroll scrollbar-hidden">
                <FormField
                            ...
                            <FormControl>
                                <>
                                    <UploadButton
                                        ...
                                        appearance={{
                                            button: "bg-indigo-600 hover:bg-indigo-700 text-white rounded-md p-2 w-fit text-center cursor-pointer h-10",
                                        }}
                                        content={{
                                            button({ ready, isUploading }:
                                                { ready: boolean; isUploading: boolean; }
                                            ) {
                                                if (ready && !isUploading)
                                                    return (
                                                        <div className="">
                                                            {field.value !== "" ? "Change" : "Add a Cover Image"}
                                                        </div>
                                                    );
                                                if (isUploading) return <div>Uploading</div>;
                                            },
                                            allowedContent({ ready, fileTypes, isUploading }:
                                                { ready: boolean; fileTypes: string[]; isUploading: boolean; }
                                            ) {
                                                if (!ready) return "";
                                                if (ready) return "";
                                    ...
                                    />
                                    {field.value !== "" && <Image src={field.value} alt="Banner" height={4} width={20} className="h-40 object-cover w-full rounded-lg" />}
                                </>
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                            ...
                            <FormControl>
                                <>
                                    <div className="flex flex-wrap gap-2">
                                        {tags.map((tag, index) => (
                                            <div key={index} className="flex items-center gap-2 bg-gray-200 rounded-md p-2">
                                                <span className="text-sm">{tag}</span>
                                                <FontAwesomeIcon icon={faCircleXmark} className="text-red-500 cursor-pointer" onClick={() => {
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            >
                                <FormControl>
                                    <SelectTrigger className="flex w-fit gap-2 bg-white border-none focus:ring-0 focus:border-none focus:ring-offset-0 focus:ring-opacity-0 focus:border-transparent focus:border-collapse active:border-collapse">
                                        <SelectValue
                                            placeholder="Post to?"
                                        />
                                    </SelectTrigger>

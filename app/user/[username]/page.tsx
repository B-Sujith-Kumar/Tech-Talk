...
const ProfilePage = async ({ params: { username } }: ProfileProps) => {
  ...
  return (
    <div className="h-full">
      <div className="h-40 bg-[#171717] relative rounded-lg"></div>
        ...
        <div className="bg-white max-w-5xl shadow-md mx-auto border border-b-0 rounded-lg relative">
          <div className="relative">
            <Image
              ...
              height={50}
              alt={user.username}
              className="aspect-square p-[8px] border-white bg-[#171717] w-24 -top-12 rounded-full absolute  left-[45%] max-sm:left-[40%] max-[430px]:left-[38%]"
            />
          </div>
                  ...
                  {posts.map((post: IPostPopulated) => (
                    <>
                      <FeedPost
                        ...
                        showBanner={false}
                        communitiesData={
                          communitiesData.status ? communitiesData.data : []
                        }
                        currentUser={currentUser}
                        key={Math.random()}
                      />
